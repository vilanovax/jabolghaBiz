          ...
          <button
            ...
            className={`shrink-0 px-3 py-1.5 rounded-lg text-[11px] font-semibold transition-colors ${
              activeTab === t.key
                ...
                : 'bg-surface-card/50 text-fg-secondary hover:text-fg'
            }`}
          >
            {t.label}
          </button>
      ...
      {activeTab === 'ranking' ? (
        <div className="space-y-3">
          <Card className="text-center py-3">
            ...
            <p className="text-xl font-black text-accent-money font-fa">
              {new Intl.NumberFormat('fa-IR').format(totalWealth)} تومان
            </p>
            <p className="text-[10px] text-fg-muted mt-0.5">{leaderboard.length} بازیکن فعال</p>
          </Card>
        ...
        <div className="space-y-3">
          ...
          {regularNews.length > 0 ? (
            ...
            <Card className="text-center py-8">
              ...
              <p className="text-xs text-fg-secondary">خبری در این دسته‌بندی نیست</p>
            </Card>
          ) : null}
        </div>
      )}
